...
static bool DirectoryExists(const char* pathname)
{
	struct stat info;
...
static int CreateDirectory(const char* pathname)
{
#ifdef _MSC_VER
	return _mkdir(pathname);
#else
	return mkdir("output", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
#endif
}
	...
	int min_distance = 60)
{
	...
	{
		dir_err = CreateDirectory("output");
		if (-1 == dir_err)
		{
	...
	{
		dir_err = CreateDirectory(output_folder.c_str());
		if (-1 == dir_err)
		{
			printf("Error creating directory !  %s\n", output_folder.c_str());
			exit(1);
	...
	if (!DirectoryExists(output_folder.c_str()))
	{
		dir_err = CreateDirectory(output_feature_folder.c_str());
		if (-1 == dir_err)
		{
